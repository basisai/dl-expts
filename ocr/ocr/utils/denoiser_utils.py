...
class SequenceGenerator:
    ...
    def generate_sequences(self, inputs, begin_states, sentence):
        ...
        for tokens in best_tokens:
            dist = leven.levenshtein(sentence, self.detokenizer(tokens))  # dist = leven.levenshtein(sentence, ' '.join(self.detokenizer(tokens)))
            if dist < best_dist:
                best_dist = dist
                best_tokens_ =[tokens]
            elif dist == best_dist:
        ...
        for tokens in best_tokens__:
            if len(tokens) > 1:
                ...
                if ppl < best_ppl:
                    ...
                    best_ppl = ppl
        output = self.detokenizer(output_tokens)  # output = ' '.join(self.detokenizer(output_tokens))
